...
// Versión inglesa: mismo slug de fichero que su par español (así el selector
// de idioma y los hreflang se emparejan por convención /blog/x ↔ /en/blog/x)
...
const guiasEn = defineCollection({
  loader: glob({ pattern: '*.md', base: './src/content/en/guias' }),
  schema: guiasSchema,
})
...
const itpEn = defineCollection({
  loader: glob({ pattern: '**/*.md', base: './src/content/en/guias/itp' }),
  schema: itpSchema,
})

export const collections = { blog, blogEn, itp, itpEn, guias, guiasEn }
